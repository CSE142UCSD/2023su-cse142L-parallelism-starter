...
struct tensor_t
{
	...
	*/
	...
	tensor_t( int _x, int _y=1, int _z=1, int _b=1, bool zero=false,  T* memory=NULL ) :  size(_x, _y, _z, _b), delete_memory(true) {
		if (memory) {
			data = memory;
			delete_memory=false;
		} else {
...
//			}
	...
	tensor_t(const tdsize & _size, bool zero=false) : size(_size), delete_memory(true)
	{
		if (size.b == 0) {
			size.b = 1;
		}
